...
public class Redescription implements Comparable<Redescription>{
    ...
    double JS=0.0, JSTrain=0.0, JSAll=0.0;
    double pVal=0.0, pValValidation=0.0;
    ...
    Redescription(DataSetCreator dat){
        ...
        for(int i=0;i<dat.W2indexs.size()+1;i++)
            viewElementsLists.add(new ArrayList<>());
    ...
    int checkAttributes(ApplicationSettings appset, Mappings map, DataSetCreator dat){
        
       int contains=0, notContained=0;
       TIntHashSet attrs=this.computeAttributesRedSet(viewElementsLists, dat);
               
        for(int i=0;i<appset.importantAttributes.size();i++)
            if(attrs.contains(map.attId.get(appset.importantAttributes.get(i))) && appset.attributeImportance==1){
                contains=1;
                return contains;
            }
            else if(!attrs.contains(map.attId.get(appset.importantAttributes.get(i))) && appset.attributeImportance==2){
                    notContained=1;
            }
    ...
    Redescription(Redescription R, DataSetCreator dat){
     ...
     Redescription(String LR, String RR, double sim, Mappings map, DataSetCreator dat, int operator){
        ...
        for(int i=0;i<dat.W2indexs.size()+1;i++)
            viewElementsLists.add(new ArrayList<>(5));
        elementsUnion=new TIntHashSet(dat.numExamples);
        JS=sim;
     ...
     void removeRedundant(){
      ...
      void disjunctiveJoinPess(Rule r, ApplicationSettings appset ,DataSetCreator dat, Mappings map,ArrayList<TIntHashSet> SideElems, int side, int negate){//0 left, 1 right  //fix memory!
		...
		if(missing==1 && fnc==1)
         ...
         if(negate==1)
             System.out.println("Missings size: "+missings.size());
        ...
        public double computeNetworkDensity(NHMCDistanceMatrix mat, ApplicationSettings appset){
                       ...
                       continue;
                                ...
                                while(itN.hasNext()){
                                    ...
                                    for(int i1=0;i1<edg.size();i1++){
                                        ...
                                        if(!this.elements.contains(t.getValue0()) || usedNodes.contains(t.getValue0()) && appset.directed == 0)
                                            continue;
            ...
            if(appset.directed == 0){
                if(mat.networkType==0)
                     ...
                     density = 2.0*sum/(this.elements.size()*(this.elements.size()-1)*mat.numLayers+((this.elements.size()*(this.elements.size()-1))*(mat.numLayers*(mat.numLayers-1))));
            }
            else{
                if(mat.networkType==0)
                     density = sum/(this.elements.size()*(this.elements.size()-1)*mat.numLayers);
                else if(mat.networkType==1)
                     density = sum/(this.elements.size()*(this.elements.size()-1)*mat.numLayers+((this.elements.size()*(this.elements.size()-1))*mat.numLayers*(mat.numLayers-1)));
            }
            System.out.println("density: "+density);
            return density;
        }
        
         public double computeAverageTriangles(NHMCDistanceMatrix mat, ApplicationSettings appset, int gs){
            double density = 0.0, sum=0.0;
            HashSet<Integer> usedNodes = new HashSet<>();
                   ...
                   double sum1 = 0.0;
                    ...
                    while(it1.hasNext()){
                        ...
                        if(elem == elem1)
                         ...
                         TIntIterator it2 = this.elements.iterator();
                         ...
                         while(it2.hasNext()){
                            ...
                            sum1+=Math.pow(mat.m_distancesS.get(elem+"#"+elem1)*mat.m_distancesS.get(elem+"#"+elem2)*mat.m_distancesS.get(elem1+"#"+elem2),1.0/3);
                        
                    }
                    //usedNodes.add(elem);
                }
                  sum+=sum1/(((this.elements.size())*(this.elements.size()-1)*(this.elements.size()-2)));  
                }

                //density = 2.0*sum/(this.elements.size()*(this.elements.size()-1));
                density = sum; //sum/this.elements.size();
                       ...
                       continue;
                                ...
                                while(itN.hasNext()){
                                    ...
                                    for(int i1=0;i1<edg.size();i1++){
                                        ...
                                        if(!this.elements.contains(elem2))
                                            continue;
                                        
                                        int layId1 = t.getValue1();
                                       HashMap<Integer,ArrayList<Triplet<Integer,Integer,Double>>> conn1 = mat.connectivityMultiplex.get(elem2);
                                       if(conn1 == null)
                                           continue;
                                       ArrayList<Triplet<Integer,Integer,Double>> edg1=conn1.get(layId1);
                                       
                                          
                                     for(int j1=i1+1;j1<edg.size();j1++){
                                          Triplet<Integer,Integer,Double> t1=edg.get(j1);
                                            int elem3 =  t1.getValue0();
                                         
                                            if(!this.elements.contains(elem3))
                                                continue;
                                          ...
                                          if(gs == 0)
                                            sum1+=Math.pow(edg.get(i1).getValue2()*edg.get(j1).getValue2()*edg1.get(cTmp).getValue2(),1.0/3)/(this.elements.size()*((this.elements.size()-2)*(this.elements.size()-1)*mat.numLayers+(mat.numLayers*(mat.numLayers-1))*(this.elements.size()-1)*(this.elements.size())+(mat.numLayers*(mat.numLayers-1))*((this.elements.size())*(this.elements.size()-1)/2)+(this.elements.size())^2*(mat.numLayers*(mat.numLayers-1)*(mat.numLayers-2)/6)));//dodaj weightove
                                          else
                                               sum1+=Math.pow(edg.get(i1).getValue2()*edg.get(j1).getValue2()*edg1.get(cTmp).getValue2(),1.0/3)/(this.elements.size()*((this.elements.size()-2)*(this.elements.size()-1)*mat.numLayers));//dodaj weightove
            ...
            System.out.println("densityTriangle: "+density);
            return density;
        }
        
         
          public double computeGlobalMeasure(NHMCDistanceMatrix mat, ApplicationSettings appset, int gs){
            double density = 0.0;
                ...
                while(it.hasNext()){
                    ...
                    if(!mat.globalNodeInfo.containsKey(elem)){
                        System.out.println("Not contained: "+elem);
                        continue;
                    }

                    density +=mat.globalNodeInfo.get(elem);
            }
                
             density/=this.elements.size();
                
            System.out.println("densityGlobal: "+density);
            return density;
        }
        
          public double  calcAvZScore(NHMCDistanceMatrix mat, ApplicationSettings appset, int gs){
                 double density = 0.0, sum=0.0;
            HashSet<Integer> usedNodes = new HashSet<>();
            ...
            if(mat.inputType==0){ //only one network
              ...
              Iterator<Integer> it1 = degree.keySet().iterator();
                double degs[] = new double[degree.keySet().size()];
                
                double avDegree = 0.0, std  =0;
                int c = 0;
                
                while(it1.hasNext()){
                    int e = it1.next();
                    avDegree+=degree.get(e);
                    degs[c++] = degree.get(e);
                }
                
                avDegree/=degree.keySet().size();
                StandardDeviation s = new StandardDeviation();
                std=s.evaluate(degs);
                
                it1 = degree.keySet().iterator();
                int c1 = 0;
                double c2 = 0.0;
                
                while(it1.hasNext()){
                    int e = it1.next();
                    if(((degree.get(e)-avDegree)/std)>2.5)
                        c1++;
                    
                    if(((degree.get(e)-avDegree)/std)>0.0)
                        c2+=((degree.get(e)-avDegree)/std);
                    
                    degree.put(e, (degree.get(e)-avDegree)/std);
                }
                        
                density=c2/this.elements.size();
  ...
  for(int k=0;k<keys.length;k++){
    ...
    TIntIterator it=toKeap.iterator();
