...
public class SupplementingRandomForest {
    public static void main(String[] args) {
        ...
        if(appset.distanceFilePaths.size()==0 && appset.useNetworkAsBackground==true || appset.computeDensity == true){
            ...
            if(appset.spatialMeasures.get(0).equals("AverageTrianglesGlobal"))
                ...
                nclMatInit.computeBetweenessCentrality(appset.graphSequence,appset);
   ...
   */
      ...
      Result.writeToFile(appset.outFolderPath+"\\"+appset.outputName+(1)+".rr", datJ, fid, startTime,numFullRed,appset, nclMatInit ,ResultsScore, coverage,oom);
      
      if( appset.spatialMatrix.size()<=0 )
            Result.writePlots(appset.outFolderPath+"\\"+"RuleData"+(1)+".csv", appset,datJ,fid);
      else Result.writePlotsNet(appset.outFolderPath+"\\"+"RuleData"+(1)+".csv", appset,datJ,nclMatInit,fid);
