...
public class IterativeGuidedTSRedescriptionsModJoin {
    ...
    public static void main(String[] args) {
       ...
       int endIndexRR=0, endIndexRR1=0;
        ...
        for(int z=0;z<percentage.length-1;z++){
       
            nARules=0; nARules1=0;
            double startPerc=percentage[z];
            double endPerc=percentage[z+1];
       ...
       // rr.extractRules(input);
        ...
        if(appset.useJoin){
            newRedescriptions=rs.createGuidedJoin(rr, rr1, jsN, appset, oldIndexRR, oldIndexRR1, RunInd, oom,fid,datJ);
            rr.removeElements(rr.newRuleIndex);
            rr1.removeElements(rr1.newRuleIndex);
        }
        else if(!appset.useJoin){
            newRedescriptions=rs.createGuidedNoJoin(rr, rr1, jsN, appset, oldIndexRR, oldIndexRR1, RunInd,oom,fid,datJ);
            rr.removeElements(rr.newRuleIndex);
            rr1.removeElements(rr1.newRuleIndex);
        }
